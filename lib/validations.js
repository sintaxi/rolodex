...
exports.presentPassword = function(field, obj, errors, next){
  ...
  if(obj.hasOwnProperty("email_verified") && obj["email_verified"] === true){
    ...
    // the key can exist holding undefined (basic-auth with no password
    // reaches here as { password: undefined }) — that is an absent
    // password, not a crash
    if(obj.hasOwnProperty("password") && obj["password"] != null && obj["password"].length > 0){
      next()
    }else{
      errors.push("must be present")
      next()
...
exports.email = function(field, obj, errors, next) {
  if(typeof obj[field] !== "string" || !validator.isEmail(obj[field])){
...
exports.verifiedEmail = function(field, obj, errors, next){
  ...
  // aleady verified
  if(obj.hasOwnProperty("email_verified_at")) return next()

  // password provided so we dont care about this
  if(obj.hasOwnProperty("password")) return next()
